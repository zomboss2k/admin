...
export default function FormUploadAvatar({
  ...
  const customRequest = async ({ onSuccess, file, resource }) => {
    ...
    if (!isImage(file.name) && previewSource) {
      const previewImageFile = await generateVideoThumbnail(file);
      const previewImageFileResponseS3 = await getUrl(
        previewImageFile.name,
        previewImageFile.type,
        resource,
      );
      await uploadMedia(previewImageFileResponseS3.uploadUrl, previewImageFile);

      form.setFieldsValue({
        [prefixSource ? `${prefixSource}.${previewSource}` : previewSource]:
          previewImageFileResponseS3.name,
      });
    }
    if (response) {
  ...
  }, [prefixSource, previewSource, record, source, form]);
