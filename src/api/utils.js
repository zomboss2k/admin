...
function requestWrapper(method) {
  const request = async (url, data = null, params = {}, prefix = '') => {
    let convertUrl = `${process.env.REACT_APP_SERVER_URL}${prefix}${url}`;
    ...
    if (prefix === '/client') {
      ...
      const key = crypto
        .createHash('md5')
        .update(`${timestamp}_${process.env.REACT_APP_HASH_KEY}`)
        .digest('hex');
      defaults.headers.timestamp = timestamp;
      defaults.headers.key = key;
    }
